...
CROSS_LAYER_CONTRACT_VERSION = "cross-layer-governance-v4"
...
MARKET_CAP_CURRENCY_POLICY_VERSION = "original-and-base-market-cap-currency-v2"
...
class CrossLayerManifest(BaseModel):
    ...
    contract_version: Literal["cross-layer-governance-v4"] = CROSS_LAYER_CONTRACT_VERSION
    ...
    calendar_alignment_policy_version: Literal["cross-layer-temporal-alignment-v1"] = (
        CALENDAR_ALIGNMENT_POLICY_VERSION
    )
    market_cap_currency_policy_version: Literal["original-and-base-market-cap-currency-v2"] = (
        MARKET_CAP_CURRENCY_POLICY_VERSION
    )
    canonicalization_version: Literal["typed-canonical-json-v1"] = CANONICALIZATION_VERSION
    runtime_fingerprint_schema_version: str
    runtime_fingerprint: str = Field(pattern=r"^[0-9a-f]{64}$")
    accounting_period_adapter_version: Literal["accounting-period-semantics-v1"] = (
        ACCOUNTING_PERIOD_ADAPTER_VERSION
    )
    value_temporal_selection_policy_version: Literal["value-fy-flow-and-period-end-instant-v1"] = (
...
def _frame_hash(frame: pd.DataFrame, keys: list[str]) -> str:
    return typed_frame_hash(frame, keys)
...
def _universe(directory: Path, cutoff: datetime.datetime) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if (source > available).any() or (available > pd.Timestamp(cutoff)).any():
        raise CrossLayerGovernanceError(
            "universe chronology violates source_timestamp <= available_at <= as_of"
        )
    symbols = tuple(
        sorted(
            membership.loc[membership["eligibility_status"] == "ELIGIBLE", "symbol"]
            .astype(str)
            .str.strip()
            .str.upper()
            .unique()
    ...
    return membership, {
        ...
        "membership": verified.membership_sha256,
        "validation": verified.validation_sha256,
        "ruleset": verified.metadata["ruleset"]["version"],
        "as_of": universe_as_of.to_pydatetime(),
        "symbols": symbols,
        "symbols_hash": eligible_symbols_hash(symbols),
    }
...
def integrate_governed_inputs(
    *,
    ...
    allowed_market = set(universe["symbols"]) | {
        str(x).strip().upper() for x in reference_symbols or set()
    }
    ...
    if set(market["symbol"].astype(str).str.upper()) != allowed_market:
        raise CrossLayerGovernanceError(
            "market symbols do not match eligible universe and explicit references"
        )
    ...
    if set(facts["entity"].astype(str).str.upper()) != set(universe["symbols"]):
        raise CrossLayerGovernanceError(
            "accounting entities do not exactly match eligible universe"
        )
    ...
    for index, row in facts.iterrows():
        ...
        if unit_kind(unit) == "MONETARY":
            market_at = (
                pd.Timestamp(row["period_end"], tz="UTC")
    ...
    for symbol in universe["symbols"]:
        ...
        if unit_kind(source_currency) != "MONETARY":
            raise CrossLayerGovernanceError(f"market_cap_currency must be monetary for {symbol}")
        market_at = pd.Timestamp(member["source_timestamp"]).to_pydatetime()
        source_amount = float(member["market_cap"])
        try:
            converted = fx.convert(
                source_amount,
            ...
            raise CrossLayerGovernanceError(
                f"market_cap FX translation failed for {symbol}: {error}"
            ) from error
        membership.loc[member_index, "market_cap"] = converted.converted_amount
        membership.loc[member_index, "market_cap_currency"] = base
        rows.append(
            {
                "entity": symbol,
                "metric": "market_cap",
    ...
    hashes = {
        "market": _frame_hash(market, ["symbol", "date"]),
        "accounting": _frame_hash(facts, ["entity", "metric", "period_end", "revision"]),
        "fx": _frame_hash(conversions, ["entity", "metric", "period_end"]),
    }
    runtime = runtime_fingerprint()
    identity = {
        "contract": CROSS_LAYER_CONTRACT_VERSION,
        ...
        "base": base,
        "market": market_data.metadata.model_dump(mode="json"),
        "fx": fx.metadata.model_dump(mode="json"),
        "accounting": accounting.metadata.model_dump(mode="json"),
        "hashes": hashes,
        "required": required,
    }
    manifest = CrossLayerManifest(
        ...
        membership_hash=universe["membership"],
        validation_hash=universe["validation"],
        eligible_symbols_hash=universe["symbols_hash"],
        eligible_symbols_count=len(universe["symbols"]),
        universe_ruleset_version=universe["ruleset"],
        ...
        market_data_snapshot_sha256=hashes["market"],
        fx_contract_version=fx.metadata.contract_version,
        fx_canonical_id=fx.metadata.canonical_id,
        fx_checksum=fx.metadata.checksum,
        fx_staleness_policy_version=fx.metadata.staleness_policy.version,
        fx_conversions_sha256=hashes["fx"],
        ...
        accounting_snapshot_sha256=hashes["accounting"],
...
def write_governed_inputs(result: CrossLayerGovernanceResult, *, output_root: Path) -> Path:
    ...
    payloads = {
        "universe_membership.csv": result.universe_membership.to_csv(
