...
def security(
    ...
    valid_from=datetime.datetime(2010, 1, 1, tzinfo=UTC),
    valid_to=None,
    available=datetime.datetime(2010, 1, 2, tzinfo=UTC),
    record="s1",
    cik="0000000001",
):
    return SecurityIdentityRecord(
        permanent_id=pid,
        issuer_id=f"issuer-{pid}",
        symbol=symbol,
        exchange="NYSE",
        listing_start=start,
        listing_end=end,
        delisting_status="DELISTED" if end else "ACTIVE",
        share_class="A",
        security_type="COMMON_STOCK",
        canonical_cik=cik,
        cik_lineage="provider-cik-field" if cik is not None else None,
        source="licensed-master",
...
def member(
    pid="sec-1",
    entry=datetime.datetime(2010, 1, 1, tzinfo=UTC),
    exit=None,
    available=datetime.datetime(2010, 1, 2, tzinfo=UTC),
    record="m1",
):
...
def test_ticker_change_preserves_identity_and_reorder_preserves_hash():
    old = security(
        ...
        valid_to=datetime.datetime(2021, 1, 1, tzinfo=UTC),
    )
    new = security(
        symbol="NEW",
        start=datetime.datetime(2021, 1, 1, tzinfo=UTC),
        valid_from=datetime.datetime(2021, 1, 1, tzinfo=UTC),
        record="s2",
    )
    membership = member()
    ...
    assert (
        reconstruct([old], [membership], OLD_DATE).artifact.artifact_hash
        == reconstruct([old], [membership], OLD_DATE).artifact.artifact_hash
    )
...
def test_listing_delisting_and_survivorship_reconstruction():
    ...
    assert reconstruct([delisted], [membership], OLD_DATE).artifact.permanent_identities == (
        "sec-1",
    )
...
def test_ticker_reuse_and_same_ticker_different_security_do_not_merge():
    ...
    second_member = member(
        pid="sec-2", entry=datetime.datetime(2022, 1, 1, tzinfo=UTC), record="m2"
    )
    assert reconstruct(
        [first, second], [first_member, second_member], OLD_DATE
    ).artifact.permanent_identities == ("sec-1",)
    assert reconstruct(
        [first, second], [first_member, second_member], NEW_DATE
    ).artifact.permanent_identities == ("sec-2",)
