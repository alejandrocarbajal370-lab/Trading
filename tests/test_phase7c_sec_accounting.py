...
def _result(tmp_path, observations, *, accepted="2026-02-01T16:00:00Z"):
    submissions = {"filings": {"recent": {
        ...
        "filingDate": ["2026-02-01"], "form": ["10-K"]}, "files": []}}
    ...
    for concept, unit, observation in observations:
        facts["facts"]["us-gaap"].setdefault(concept, {"units": {}})["units"].setdefault(unit, []).append({
            ...
            "val": observation.get("val", 100), "accn": ACCESSION,
...
def test_sec_to_accounting_is_issuer_level_sealed_and_confidence_open(tmp_path):
    result = bind_sec_to_accounting(_result(tmp_path, [_revenue()]), plan=_plan(tmp_path, two_classes=True), as_of=AS_OF)
...
def test_unregistered_custom_or_alias_is_unmapped_never_guessed(tmp_path):
    ...
    bound = bind_sec_to_accounting(sec, plan=_plan(tmp_path), as_of=AS_OF)
...
@pytest.mark.parametrize("column,value,match", [
    ("unit", "SHARES", "unit family"),
    ("period_type", "instant", "instant/duration"),
    ("fiscal_period_start", "2025-12-31", "FY duration"),
    ("available_at", pd.Timestamp("2026-08-26T00:00:00Z"), "exceeds cutoff"),
    ("raw_snapshot_sha256", "0" * 64, "raw snapshot"),
])
def test_semantic_and_proof_mutations_fail_closed(tmp_path, column, value, match):
    ...
    with pytest.raises(SecAccountingBindingError, match=match):
        bind_sec_to_accounting(sec, plan=_plan(tmp_path), as_of=AS_OF)
...
def test_registry_mutation_even_resealed_requires_explicit_equivalence(tmp_path):
    ...
    with pytest.raises(ValueError, match="hash mismatch"):
        bind_sec_to_accounting(_result(tmp_path, [_revenue()]), plan=_plan(tmp_path), as_of=AS_OF, registry=stale)
...
def test_multiple_frames_for_same_accession_are_ambiguous(tmp_path):
    with pytest.raises(SecAccountingBindingError, match="ambiguous SEC"):
        bind_sec_to_accounting(
            ...
            plan=_plan(tmp_path), as_of=AS_OF)
...
def test_sec_accounting_snapshot_before_and_after_amendment(tmp_path):
    sec = _result(tmp_path, [_revenue(100)])
    amendment = sec.facts.iloc[0].copy()
    amendment["accession"] = "0000000001-26-000002"
    amendment["form"] = "10-K/A"
    amendment["value"] = 105
    amendment["available_at"] = pd.Timestamp("2026-07-01T16:00:00Z")
    sec.facts.loc[len(sec.facts)] = amendment
    bound = bind_sec_to_accounting(sec, plan=_plan(tmp_path), as_of=AS_OF)
...
def test_currency_is_preserved_and_qvm_remains_blocked_without_fx(tmp_path):
    cny = list(_revenue())
    cny[1] = "CNY"
    bound = bind_sec_to_accounting(_result(tmp_path, [tuple(cny)]), plan=_plan(tmp_path), as_of=AS_OF)
