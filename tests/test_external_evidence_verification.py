...
def valid_inputs(
    *,
    authority_delta=dt.timedelta(hours=3),
    ...
    authorities, receipts, decisions, revocations, observed_materials = [], [], [], [], []
    for index, expectation in enumerate(manifest.expectations):
        observed = observe_contract_test_material(
            gate=expectation.gate,
            material=contract_test_material_for_gate(expectation.gate),
            observed_at=NOW - evidence_age,
            adapter=adapter,
        )
        ...
        receipt = seal(
            ...
            signature_check="MATCHED_UNTRUSTED",
        )
        authority = seal(
            VerifierAuthoritySnapshot,
            "snapshot_hash",
        ...
        decision = seal(
            ...
            verifier_time=NOW,
            made_at=NOW - dt.timedelta(minutes=40),
        ...
        revocation = seal(
            ...
            verifier_time=NOW,
            reviewed_at=NOW - revocation_delta,
        ...
        observed_materials.append(observed)
...
def reseal_chain(values, index, **receipt_updates):
    adapter, authorities, receipts, decisions, revocations, observed_materials = values
    receipt = reseal(receipts[index], "receipt_hash", **receipt_updates)
    assessment = typed_hash(
        {
            "contract": CONTRACT_VERSION,
            "gate": receipt.gate.value,
            "expectation_hash": receipt.expectation_hash,
            "provider_receipt_digest": receipt.provider_receipt_digest,
            "verifier_time": NOW.isoformat(),
        }
    )
    receipt = reseal(receipt, "receipt_hash", assessment_identity=assessment)
    authority = reseal(
        authorities[index],
        "snapshot_hash",
        gate=receipt.gate,
...
def test_complete_cross_gate_package_swap_fully_resealed_fails():
    ...
    def relabel_and_reseal(package, target_gate):
        ...
        receipt = reseal(
            old_receipt, "receipt_hash", gate=target_gate, assessment_identity=assessment
        )
        authority = reseal(
            old_authority,
            "snapshot_hash",
    ...
    with pytest.raises(ExternalEvidenceVerificationError):
        assess((adapter, authorities, receipts, decisions, revocations, observed_materials))


def test_auditor_exact_fully_resealed_swap_recomputes_material_and_fails():
    values = list(valid_inputs())
    manifest = canonical_gate_verification_manifest()
    source_observed = values[5][0]
    target = manifest.expectations[1]
    forged_observation = source_observed.model_dump(mode="python")
    forged_observation.update(
        gate=target.gate,
        expectation_hash=target.expectation_hash,
        provider_ref=target.provider_ref,
        dataset_ref=target.dataset_ref,
        dataset_version_ref=target.dataset_version_ref,
        adapter_ref=target.adapter_ref,
        adapter_release_ref=target.adapter_release_ref,
        evidence_policy_ref=target.evidence_policy_ref,
        receipt_policy_ref=target.receipt_policy_ref,
        material_digest=target.expected_artifact_digest,
    )
    forged_observation.pop("observation_hash")
    forged_observation["observation_hash"] = typed_hash(forged_observation)
    values[5][1] = type(source_observed).model_construct(**forged_observation)
    receipt_updates = {
        "gate": target.gate,
        "expectation_hash": target.expectation_hash,
        "artifact_digest": target.expected_artifact_digest,
        "observation_hash": forged_observation["observation_hash"],
        "provider_receipt_digest": typed_hash({"fully": "resealed-receipt"}),
        "replay_nonce_digest": typed_hash({"fully": "resealed-nonce"}),
    }
    values = list(reseal_chain(tuple(values), 1, **receipt_updates))
    with pytest.raises(ExternalEvidenceVerificationError, match="observed material"):
        assess(tuple(values))


def test_changed_bytes_with_wrong_canonical_provenance_fails():
    values = list(valid_inputs())
    adapter = values[0]
    target_gate = tuple(EvidenceGate)[1]
    observed = observe_contract_test_material(
        gate=target_gate,
        material=contract_test_material_for_gate(target_gate, "v2"),
        observed_at=values[2][1].observed_at,
        adapter=adapter,
    )
    values[5][1] = reseal(
        observed,
        "observation_hash",
        provider_ref=canonical_gate_verification_manifest().expectations[0].provider_ref,
    )
    values = list(
        reseal_chain(
            tuple(values),
            1,
            artifact_digest=observed.material_digest,
            observation_hash=values[5][1].observation_hash,
        )
    )
    with pytest.raises(ExternalEvidenceVerificationError, match="canonical binding"):
        assess(tuple(values))
...
def test_same_material_renamed_and_fully_resealed_is_still_replay():
    ...
    for index in range(len(tuple(EvidenceGate))):
        second = reseal_chain(
            second,
            index,
            provider_receipt_digest=typed_hash({"renamed-receipt": index}),
...
def test_genuinely_distinct_canonically_registered_material_passes():
    ...
    for index, gate in enumerate(EvidenceGate):
        observed = observe_contract_test_material(
            gate=gate,
            material=contract_test_material_for_gate(gate, "v2"),
            observed_at=values[2][index].observed_at,
            adapter=adapter,
        )
        values[5][index] = observed
        values = list(
            reseal_chain(
                tuple(values),
                index,
...
def test_forged_nested_observation_and_sequential_atomic_consume_fail():
    values = list(valid_inputs())
    forged = values[5][0].model_copy(update={"material_digest": "f" * 64})
    values[5][0] = forged
    with pytest.raises(ExternalEvidenceVerificationError, match="observed material"):
        assess(tuple(values))
    ledger = InMemoryContractTestReplayLedger()
    identity = typed_hash({"atomic": "identity"})
    ledger.consume_many((identity,), NOW)
    with pytest.raises(ExternalEvidenceVerificationError, match="replayed"):
        ledger.consume_many((identity,), NOW)
...
def test_forged_manifest_or_registry_metadata_cannot_be_supplied(field):
    ...
    with pytest.raises(ExternalEvidenceVerificationError, match="invalid adapter"):
        assess((extra, authorities, receipts, decisions, revocations, observed_materials))


@pytest.mark.parametrize("collection_index", [1, 2, 3, 4, 5])
...
def test_reorder_is_canonicalized_without_positional_authority():
    ...
    result = assess(
        (
            adapter,
            list(reversed(authorities)),
            list(reversed(receipts)),
            list(reversed(decisions)),
            list(reversed(revocations)),
            list(reversed(observed_materials)),
        )
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "revoked_before_observation",
        "revoked_after_decision",
        "authority_unknown",
        "revocation_unknown",
        "stale_hash",
    ],
...
def test_authority_revocation_causality_and_status_fail_closed(kind):
    ...
    if kind == "authority_365d":
        ...
        revocations[0] = reseal(
            revocations[0], "review_hash", reviewed_at=NOW - dt.timedelta(days=365)
        )
    elif kind == "authority_after_observation":
        authorities[0] = reseal(
            authorities[0], "snapshot_hash", captured_at=NOW - dt.timedelta(minutes=30)
        )
        ...
        revocations[0] = reseal(revocations[0], "review_hash", status="REVOKED", revoked_at=when)
...
SECRET_PAYLOADS = [
    "token.secretvalue1234567890",
    "sk_live_0123456789abcdef",
    "user:password",
    "a" * 64,
    "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxIn0.signature",
    "c2VjcmV0LWxvY2F0b3ItdmFsdWU=",
...
def test_secret_metadata_fields_are_not_representable_or_serialized(field, payload):
    ...
    with pytest.raises(ExternalEvidenceVerificationError, match="invalid adapter"):
        assess((injected, authorities, receipts, decisions, revocations, observed_materials))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("real_route", "QVM_READY"),
        ("live_execution_enabled", True),
        ("gate_states", tuple((gate, GateState.VERIFIED) for gate in EvidenceGate)),
    ],
)
def test_promoted_result_cannot_cross_public_truth_boundary(transport, field, promoted):
    ...
    if transport == "copy":
        ...
        forged = json.loads(
            ExternalVerificationFoundationResult.model_construct(**values).model_dump_json(
                warnings=False
            )
        )
...
def test_replay_and_receipt_freshness_boundaries(attack):
    ...
    if attack == "nonce":
        receipts[1] = reseal(
            receipts[1], "receipt_hash", replay_nonce_digest=receipts[0].replay_nonce_digest
        )
    elif attack == "receipt":
        receipts[1] = reseal(
            receipts[1], "receipt_hash", provider_receipt_digest=receipts[0].provider_receipt_digest
        )
    elif attack == "stale":
...
def test_exact_24h_receipt_age_is_allowed_when_not_expired():
    result = assess(
        valid_inputs(authority_delta=dt.timedelta(hours=24), evidence_age=dt.timedelta(hours=24))
    )
    assert len(result.candidates) == 10
...
def test_naive_time_and_extra_secret_field_rejected():
    ...
    with pytest.raises(ValidationError):
        VerifierAuthoritySnapshot.model_validate(
            {
                **authorities[0].model_dump(mode="python"),
                "credential": "secret",
            }
        )
