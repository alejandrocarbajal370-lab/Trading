# ADR 0011 — IBKR Reproducible Read-Only REAL Observation Probe

Status: **AUTHORIZED_TO_IMPLEMENT; EXPLICIT_LOCAL_REAL_OBSERVATION_ONLY; PROVIDER ADMISSION NOT_AUTHORIZED**

## Decision

After PR #34, the user authorized exactly one successor: **IBKR Reproducible Read-Only REAL
Observation Probe** under `NEW_PR_REQUIRED` and `AFTER_CURRENT_BLOCK_MERGED`. It may run only by an
...
caller provenance claims. The REAL entry point captures the concrete local adapter in a lexical
boundary and attaches a boundary-only attestation over the complete evidence payload. It does not
use class/module introspection as a trust primitive. Automatic account-list callbacks are discarded
...
internally computed raw/material/provenance/evidence digests. A timeout never becomes a price,
...
is raised, including exception cause/context.
