# ADR 0020 — CORSO Human-Only Execution and Canonical Portfolio Ledger
...
CORSO may propose trades and rebalances for human review, but it must not submit, modify, cancel
...
automatic/live broker execution by CORSO, including language that describes live execution as a
...
| --- | --- |
| IBKR | Production `MARKET_DATA_RESEARCH` observation source; read-only access to the user's/CORSO real account where applicable |
| IBKR Paper | Temporary paper-trading, testing and validation environment only |
| Broker X | Future real-capital broker/custodian and source of portfolio/custody truth; manual execution outside CORSO |
| PostgreSQL | Intended canonical portfolio ledger, state and truth consumed by CORSO |
| Google Sheets, CSV, XLSX and manual entry | Ingestion interfaces only; never canonical before validation and reconciliation |
| CORSO | Analytics, research, monitoring, decision support and dashboard; no funds or order authority |
| Human | Sole final execution authority |

IBKR is not CORSO's real custody or execution destination under this mandate. CORSO must not
implement a production IBKR order adapter. Broker X will provide real custody and real execution,
but execution is `HUMAN_ONLY` and manual outside CORSO. CORSO will have no Broker X API for
...
grant CORSO execution or funds authority.

## Canonical portfolio architecture

PostgreSQL is the intended canonical portfolio ledger/state/truth for CORSO. Imports from Broker X
statements, CSV, XLSX, Google Sheets or manual entry are untrusted ingestion inputs until validated
and reconciled. CORSO and its dashboard consume the canonical layer read-only; the dashboard must
...
```text
IBKR market/research -> governed ingestion -> canonical data layer -> CORSO -> Dashboard
